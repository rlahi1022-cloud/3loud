...
int main(int argc, char** argv) {                                                     // main 시작
    ...
    while (g_running.load()) {                                                        // 메인 루프
        ...
        for (int i = 0; i < n; ++i) {                                                 // 이벤트 순회
            ...
            if (events[i].events & EPOLLIN) {                                         // 읽기 이벤트면
                // length-prefix 수신은 packet_recv가 blocking 전제라서                 // 설명 주석
                // epoll 기반에서는 “직접 버퍼링”이 더 정석인데,                         // 설명 주석
                // 지금은 뼈대 단계라 '단순화'로 1패킷 단위 recv를 구현한다               // 설명 주석
                // 실전 적용 시: read_buffer를 세션에 두고 프레이밍 파싱으로 바꿀 것      // 설명 주석

                uint32_t net_len = 0;                                                 // 길이 버퍼
                int r = recv(fd, &net_len, sizeof(net_len), MSG_DONTWAIT);            // 길이 4바이트 읽기 시도
                if (r == 0) {                                                         // 정상 종료
                    safe_close(fd);                                                   // close
                    sessions.erase(fd);                                               // 세션 제거
                    continue;                                                         // 다음
                }                                                                     // if 끝
                if (r < 0) {                                                          // 에러
                    if (errno == EAGAIN || errno == EWOULDBLOCK) {                    // 아직 없음
                        continue;                                                     // 다음
                    }                                                                 // if 끝
                    safe_close(fd);                                                   // close
                    sessions.erase(fd);                                               // 세션 제거
                ...
                if (r != 4) {                                                         // 4바이트 못읽음(단순화 한계)
                    safe_close(fd);                                                   // close
                    sessions.erase(fd);                                               // 세션 제거
                    continue;                                                         // 다음
                }                                                                     // if 끝

                uint32_t len = ntohl(net_len);                                        // 길이 변환
                if (len == 0 || len > static_cast<uint32_t>(MAX_PACKET_SIZE)) {      // 길이 검증
                    safe_close(fd);                                                   // close
                    sessions.erase(fd);                                               // 제거
                    continue;                                                         // 다음
                }                                                                     // if 끝

                std::string payload;                                                  // payload 저장
                payload.resize(len);                                                  // 크기 확보

                size_t got = 0;                                                       // 받은 바이트
                while (got < len) {                                                   // 다 받을 때까지
                    int n2 = recv(fd, &payload[got], len - got, MSG_DONTWAIT);        // 읽기
                    if (n2 == 0) {                                                    // 종료
                        safe_close(fd);                                               // close
                        sessions.erase(fd);                                           // 제거
                        break;                                                        // 탈출
                    ...
                    if (n2 < 0) {                                                     // 에러
                        if (errno == EAGAIN || errno == EWOULDBLOCK) {                // 아직 데이터 덜 옴
                            // 여기 단순화의 한계: 세션 read_buffer가 없어서             // 설명 주석
                            // “부분 수신”을 제대로 처리 못함                            // 설명 주석
                            // 실제 적용에서는 반드시 read_buffer 기반으로 바꿀 것       // 설명 주석
                            safe_close(fd);                                           // close(현재 뼈대에서는 보수적으로 종료)
                            sessions.erase(fd);                                       // 제거
                            break;                                                    // 탈출
                        }                                                             // if 끝
                        safe_close(fd);                                               // close
                    ...
                    got += static_cast<size_t>(n2);                                   // 누적
                }                                                                     // while 끝

                if (sessions.find(fd) == sessions.end()) {                            // 위에서 제거됐으면
                    continue;                                                         // 다음
                }                                                                     // if 끝

                {                                                                     // 요청 큐 push 블록
                    std::lock_guard<std::mutex> lk(g_req_m);                          // lock
                    g_req_q.push(Task{fd, payload});                                  // push
                }                                                                     // 블록 끝
                g_req_cv.notify_one();                                                // worker 깨우기
            }                                                                         // EPOLLIN 처리 끝
